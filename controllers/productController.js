...
const productController = {
    ...
    allTransactionsByMonth: async (req, res) => {
        try {
            let req_month = monthsLong[req.params.month];
    ...
    allTransactionsBySearch: async (req, res) => {
        try {
            let req_word = req.params.word;
            let req_month = monthsLong[req.params.month];
            const allTransaction = await productModel.find({ month: req_month }).exec();

            let filtered_search = allTransaction.filter((e) => {
                return e.title.toLowerCase().includes(req_word.toLowerCase()) || e.description.toLowerCase().includes(req_word.toLowerCase());
            })
